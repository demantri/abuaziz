<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display:none" id="modal-form-pembelian">
	<div class="sparkline13-list">
		...
		<div class="sparkline13-graph">
			<div class="datatable-dashv1-list custom-datatable-overright">
				<form action="#" class="js-form-pembelian form" id="form-pembelian">
				<div class="row" >
					<div class="col-lg-3" >
						<div class="panel panel-primary" >
							<div class="panel-heading"><i class='fa fa-file-text-o fa-fw'></i> Informasi Pembelian</div>
							<div class="panel-body">
								<label for="email_address">Tanggal Pembelian</label>
								...
								<label for="email_address">Nama Suppliar</label>
								<div class="form-group">
									<div class="form-line">
										<select name="no_supplier" id="no_supplier" data-validate-field="no_supplier" class="form-control">
...
<!-- basic table -->
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="display:true" id="modal-data-pembelian">
	<div class="sparkline13-list">
		<div class="sparkline13-hd">
			<div class="main-sparkline13-hd">
		...
		<div class="sparkline13-graph">
			<div class="datatable-dashv1-list custom-datatable-overright">
				<button type="button" class="btn btn-sm btn-primary" name="tambah_pembelian" onclick="tambah()">Tambah <?php echo $judul; ?></button>
				<br/>
				<br/>
...
$(window).ready(function(){
	...
	function tambah() {
	  $.ajax({
			type: 'POST',
			url: "<?= site_url('pembelian/list_supplier/'); ?>",
			data: {'no_akses':"NULL"},
			cache: false,
			success: function(msg){
				$("#no_supplier").html(msg);
			}
		});
      $('#modal-form-pembelian').fadeIn('slow');
      $('#modal-data-pembelian').fadeOut('slow');
	  $('#form-title').html("Tambah Pembelian");
	  $('#aksi').val("tambah");
    }
	
	function batal() {
      $('#modal-form-pembelian').fadeOut('slow');
      $('#modal-data-pembelian').fadeIn('slow');
	  $('#form-title').html("");
	  $('#form-pembelian')[0].reset();
    }
	
	// function edit(no_pembelian){
		// $.ajax({
			// url:'<?= site_url(); ?>/pembelian/data/',
			// data: {'no_pembelian':no_pembelian},
            // type:'POST',
			// dataType:'JSON',
			// success:function(msg){
				// $('input[name="aksi"]').val(msg.no_pembelian);
				// $('input[name="nama_pembelian"]').val(msg.nama_pembelian);
				// $('input[name="alamat_pembelian"]').val(msg.alamat_pembelian);
	...
	new window.JustValidate('.js-form-pembelian', {
        rules: {
            tanggal_transaksi: {
                required: true
            },
		...
		if(aksi=="tambah"){
			var url = "<?= site_url('pembelian/tambah'); ?>";
		}else{
			var url = "<?= site_url('pembelian/ubah'); ?>";
		}
			var Cash = $('#UangCash').val();
			var TotalBayar = $('#TotalBayarHidden').val();
